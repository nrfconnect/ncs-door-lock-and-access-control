...
public:
	...
	struct Callbacks {
		 ...
		 *
		 * @param data Pointer to the received ranging data.
		 * @param length Size of the received data in bytes.
		 */
	 ...
	 * @return ALIRO_NO_ERROR on success, or an error code on failure.
	 */
	AliroError ConfigureRangingSession(SessionIdentifier sessionId, const uint8_t *ursk, size_t urskLen,
					   void *sessionUserData)
	{
		return Impl()->_ConfigureRangingSession(sessionId, ursk, urskLen, sessionUserData);
	}
