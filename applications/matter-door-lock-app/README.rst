...
.. toggle::
   ...
   See :ref:`door_lock_nus_service` for the reusable NUS wrapper, Kconfig options, and the ``bt_nus`` snippet.
...
.. toggle::
   ...
   * Simple Management Protocol (SMP) over Bluetooth LE.
   ...
   Additionally, you can enable the DFU over SMP by building with the ``dfu_smp`` snippet (for example, add ``-Dmatter-door-lock-app_SNIPPET=dfu_smp`` to your CMake arguments).
   See :ref:`door_lock_app_ble_smp` and :ref:`door_lock_dfu_smp_service` for transport details, shell commands, and Kconfig options.
   ...
   Press Button 3 to toggle it, or use the ``dfu_smp on`` / ``dfu_smp off`` shell commands when shell support is enabled.
   ...
   * nRF54L15 DK
   ...
   .. tabs::
      ...
      .. group-tab:: |nRFVSC|
         ...
         When building with |nRFVSC|, add ``-Dmatter-door-lock-app_SNIPPET=dfu_smp`` to :guilabel:`Extra CMake arguments` to enable DFU over SMP.

      .. group-tab:: Command line

         When building on the command line, run the following command with *board_target* replaced with the board target name of the hardware platform you are using (see `Requirements`_):

         .. parsed-literal::
            :class: highlight

            west build -b *board_target* -- -Dmatter-door-lock-app_SNIPPET=dfu_smp

         For example:

         .. code-block:: console

            west build -b nrf54l15dk/nrf54l15/cpuapp -- -Dmatter-door-lock-app_SNIPPET=dfu_smp
...
.. toggle::
    ...
    .. tabs::
       ...
       .. group-tab:: |nRFVSC|
          ...
          Add ``-Dmatter-door-lock-app_SNIPPET=dfu_smp -DFILE_SUFFIX=internal`` to :guilabel:`Extra CMake arguments` in your build configuration.

       .. group-tab:: Command line

          .. code-block:: console

             west build -p -b nrf54lm20dk/nrf54lm20b/cpuapp -- -Dmatter-door-lock-app_SNIPPET=dfu_smp -DFILE_SUFFIX=internal
    ...
    .. tabs::
       ...
       .. group-tab:: |nRFVSC|
          ...
          Add ``-Dmatter-door-lock-app_SNIPPET=dfu_smp -DFILE_SUFFIX=internal -Dmatter-door-lock-app_EXTRA_CONF_FILE=prj_release.conf`` to :guilabel:`Extra CMake arguments` in your build configuration.

          .. code-block:: console

             west build -p -b nrf54lm20dk/nrf54lm20b/cpuapp -- -Dmatter-door-lock-app_SNIPPET=dfu_smp -DFILE_SUFFIX=internal -Dmatter-door-lock-app_EXTRA_CONF_FILE=prj_release.conf
...
.. toggle::
    ...
    See :ref:`door_lock_nus_service` for Kconfig options and usage.
...
Button 1:
   ...
   * If pressed for less than three seconds:
     ...
     * If the device is not provisioned to the Matter network, it initiates Bluetooth LE advertising for Matter commissioning.
...
.. |sample path| replace:: :file:`ncs-door-lock-and-access-control/applications/matter-door-lock-app`
...
Some of them require a different build configuration to enable.
...
.. toggle::
   ...
   #. Reboot the device.
   #. Wait until the device has rebooted and rejoined the Matter network.
...
.. toggle::
   ...
   #. Set the example ``Week-day`` schedule using the following command:
      ...
      * *end-minute* is the ending minute for the week day schedule.
...
.. toggle::
   ...
   #. Tap :guilabel:`Connect`.
      The application scans for nearby Bluetooth LE devices.
   ...
   #. Tap the generated macros to change the lock state.
