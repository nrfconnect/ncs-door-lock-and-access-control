...
bool AccessManagerImpl::VerifyPublicKey(const CryptoTypes::PublicKey &userPublicKey)
{
	...
	if (IsPublicKeyStored(mAcKeys, userPublicKey)) {
		return true;
	}

#if CONFIG_DOOR_LOCK_STORAGE_MAX_STORED_ACCESS_DOCUMENTS > 0
	size_t keyIndex{};
	if (IsPublicKeyStored(mAdKeys, userPublicKey, &keyIndex)) {
		AccessDocument ad;
		const auto error = ReadAccessDocument(keyIndex, ad);
		...
		if (!IsStoredAccessDocumentValid(ad)) {
			...
			_RemovePublicKey(PublicKeyType::AccessDocument, keyIndex, true);
			return false;
		}
		return true;
	}
