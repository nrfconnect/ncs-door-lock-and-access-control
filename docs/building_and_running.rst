...
In the workspace ``door-lock-workspace`` the |APP_NAME| is placed in the ``ncs-door-lock-app`` directory.
To build and run the application on the `nRF54L15 DK`_, complete the following steps:

1. Connect the nRF54L15 DK to your computer using the ``DEBUGGER`` port on the DK. Set the ``POWER`` switch to ``ON``.

#. In the ``door-lock-workspace`` directory, navigate to the ``ncs-door-lock-app`` folder.

#. Depending on the :ref:`NFC reader expansion board <nfc_reader>` connected to the development kit,
   build the application by running one of the following commands:
...
#. To verify the application runs, connect to the DK with a terminal emulator that supports VT100/ANSI
   escape characters (for example, nRF Connect Serial Terminal).
