...
The |ALIRO_APP_NAME| runs on Nordic Semiconductor :ref:`supported SoCs <hw_requirements>` and uses the Aliro stack for access protocol and communication with a User Device over Near Field Communication (NFC) or Bluetooth® LE (paired with ultra-wideband).
