...
This page describes how the application code can interact with the Aliro stack implementation, and shows examples of specific interaction flows.
...
For the Aliro stack architecture, the stack library location, and where interfaces are implemented, see :ref:`aliro_integration`.
...
* Aliro Stack Public API – Represents the public API defined by the Aliro stack.
...
* The NFC/Bluetooth LE Radio receives data and passes it to the application.
  ...
  For session data, the application does not interpret the protocol, but it forwards data to the stack.
...
The application provides the implementation of Transport and other platform abstractions.
