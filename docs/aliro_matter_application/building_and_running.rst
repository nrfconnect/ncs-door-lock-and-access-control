...
#. Depending on the :ref:`NFC reader expansion board <hw_requirements_nfc_reader>` connected to the development kit, build the application by running the corresponding command:
   ...
   .. list-table::
      ...
      * - Debug (default)
        - `X-NUCLEO-NFC09A1`_
        - ``west build -p -b build_target applications/matter-aliro-door-lock-app -- -DCONFIG_ST25R200_DRV=y``
...
#. To build the application with Bluetooth LE transport and UWB, run:
   ...
   .. code-block:: bash
      ...
      west build -p -b nrf5340dk/nrf5340/cpuapp applications/matter-aliro-door-lock-app -- -DCONFIG_DOOR_LOCK_BLE_UWB=y
...
.. note::
   ...
   Enable it with the ``uwb_qm35`` application snippet (see :ref:`matter_building_with_uwb`).
...
#. Refer to the build variants for additional transports and protocols:
   ...
   .. list-table::
      ...
      * - Matter and Aliro (NFC only)
        - ``west build -p -b nrf5340dk/nrf5340/cpuapp applications/matter-aliro-door-lock-app``
      * - Matter and Aliro with QM35825 UWB module
        - ``west build -p -b nrf5340dk/nrf5340/cpuapp applications/matter-aliro-door-lock-app -- -Dmatter-aliro-door-lock-app_SNIPPET=uwb_qm35``
...
Note that shell commands availability differs by build type:

* In the release builds, the UART shell is disabled by default to save memory.
* In the debug builds, both Matter shell (``matter``) and door lock shell (``dl``) remain enabled to allow testing and provisioning (see :ref:`Provisioning with CLI <testing_provisioning_cli>`).
...
#. Verify if the application runs correctly:
   ...
   .. tabs::
      ...
      .. tab:: Release configuration
         ...
         * Connect to the serial console and verify that the ``dl`` shell command is available and responds (see :ref:`Provisioning with CLI <testing_provisioning_cli>`).
         * Verify the LED and button behavior described in :ref:`matter_ui`.

.. _matter_building_with_uwb:
