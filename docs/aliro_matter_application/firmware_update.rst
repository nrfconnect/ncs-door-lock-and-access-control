...
By building the application with the ``-Dmatter-aliro-door-lock-app_SNIPPET=dfu_smp`` option, you can enable this feature and update your device's firmware wirelessly.
...
You can configure it in the :file:`applications/matter-aliro-door-lock-app/VERSION` file.
...
.. note::
   ...
   UWB builds also require the ``nrfconnect-sdk-qorvo`` west add-on (see :ref:`matter_building_with_uwb`).
   For applications that need Bluetooth LE SMP DFU support, the ``dfu_smp`` snippet must also be enabled.

.. code-block:: console

   west build -b nrf5340dk/nrf5340/cpuapp applications/matter-aliro-door-lock-app -- -Dmatter-aliro-door-lock-app_SNIPPET='uwb_qm35;uwb_qm35_dfu;dfu_smp'
