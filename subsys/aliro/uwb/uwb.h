...
public:
	...
	/**
	 * @brief Schedules start of the UWB radar session.
	 *
	 * While a CCC ranging session is active, radar may also be started automatically
	 * from valid controlee distance reports, or based on a different trigger.
	 *
	 * @note This method assumes that there is only one radar session active at a time.
	 *
	...
	/**
	 * @brief Stops the UWB radar session and cancels any pending radar start.
	 *
	 * @note This method assumes that there is only one radar session active at a time.
	 */
