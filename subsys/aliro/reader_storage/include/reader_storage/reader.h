...
/**
 * @brief Callback type invoked when Reader data changes.
 *
 * Called after Reader data is modified through shell commands when
 * `CONFIG_DOOR_LOCK_ALIRO_READER_STORAGE_SHELL` is enabled.
 *
 * Implementations should return quickly and avoid blocking operations.
 */
using ReaderDataChangedCallback = void (*)(void);
 ...
 *
 * @param dataChangedCallback Optional callback invoked after shell-triggered data changes.
 * @return ALIRO_NO_ERROR on success, error code otherwise.
 */
AliroError Init(ReaderDataChangedCallback dataChangedCallback = nullptr);
...
/**
 * @brief Gets the Reader private key ID.
 *
 * @return The Reader private key ID.
 */
...
/**
 * @brief Gets the Group Resolving Key ID.
 *
 * @return The Group Resolving Key ID.
 */
