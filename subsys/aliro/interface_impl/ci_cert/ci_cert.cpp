...
LOG_MODULE_REGISTER(interface_ci_cert, CONFIG_DOOR_LOCK_ALIRO_INTERFACE_IMPL_CI_CERT_LOG_LEVEL);
...
AliroError VerifyKeyType(const mbedtls_pk_context *pk)
{
	...
	VerifyOrReturnValue(psa_get_key_bits(&attributes) == kEccP256KeyBits, ALIRO_INVALID_DATA_CONTENT,
...
AliroError VerifyCertificateSignature(const mbedtls_x509_crt &crt)
{
	...
	VerifyOrReturnValue(result == 0, ALIRO_ERROR_INTERNAL, LOG_ERR("Failed to convert signature from DER to raw"));
	VerifyOrReturnValue(rawLength == signatureArray.size(), ALIRO_INVALID_DATA_FORMAT,
			    LOG_ERR("Invalid signature length"));

	return DoorLock::CryptoUtils::VerifySignature(
		DoorLock::InterfaceImpl::CiCert::GetCredentialIssuerCAPublicKeyId(), crt.tbs.p, crt.tbs.len,
		signatureArray);
}

AliroError VerifyCertificate(const mbedtls_x509_crt &crt)
{
	VerifyOrReturnValue(crt.version == 3, ALIRO_INVALID_DATA_FORMAT, LOG_ERR("Invalid certificate version"));

	auto result = mbedtls_x509_crt_has_ext_type(&crt, MBEDTLS_X509_EXT_KEY_USAGE);
	VerifyOrReturnValue(result != 0, ALIRO_INVALID_DATA_CONTENT, LOG_ERR("Key usage extension is not present"));
	result = mbedtls_x509_crt_check_key_usage(&crt, MBEDTLS_X509_KU_DIGITAL_SIGNATURE);
	VerifyOrReturnValue(result == 0, ALIRO_INVALID_DATA_CONTENT, LOG_ERR("Digital Signature bit not set"));

	auto error = VerifyKeyType(&crt.pk);
	VerifyOrReturnValue(error == ALIRO_NO_ERROR, error, LOG_ERR("Failed to verify key type"));
	...
	VerifyOrReturnValue(error == ALIRO_NO_ERROR, error, LOG_ERR("Failed to verify certificate signature"));
...
AliroError VerifyCertificateValidityPeriod(const CertificateTimestamps &timestamps)
{
	...
	case DoorLock::InterfaceImpl::CiCert::ValidityPeriodVerificationResult::NotSupported:
		...
		return ALIRO_NO_ERROR;
	case DoorLock::InterfaceImpl::CiCert::ValidityPeriodVerificationResult::TimeUnavailable:
		LOG_ERR("Current time not available");
		return ALIRO_ERROR_INTERNAL;
	case DoorLock::InterfaceImpl::CiCert::ValidityPeriodVerificationResult::OutsidePeriod:
		return ALIRO_PUBLIC_KEY_EXPIRED;
	case DoorLock::InterfaceImpl::CiCert::ValidityPeriodVerificationResult::WithinPeriod:
		return ALIRO_NO_ERROR;
	}
		    ...
		    std::optional<CertificateTimestamps> &timestamps)
{
	...
	VerifyOrReturnValue(certificate.mData != nullptr, ALIRO_INVALID_ARGUMENT, LOG_ERR("Certificate data is null"));
	VerifyOrReturnValue(certificate.mLength != 0, ALIRO_INVALID_ARGUMENT, LOG_ERR("Certificate length is zero"));
