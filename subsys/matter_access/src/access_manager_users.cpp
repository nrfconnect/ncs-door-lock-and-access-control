...
{
	/* userIndex is guaranteed by the caller to be between 1 and CONFIG_DOOR_LOCK_MATTER_ACCESS_MAX_NUM_USERS */
	VerifyOrReturnError(IN_RANGE(userIndex, 1, CONFIG_DOOR_LOCK_MATTER_ACCESS_MAX_NUM_USERS), false);
					   ...
					   const CredentialStruct *credentials, size_t totalCredentials)
{
	...
	VerifyOrReturnError(totalCredentials <= CONFIG_DOOR_LOCK_MATTER_ACCESS_MAX_NUM_CREDENTIALS_PER_USER, false);
	...
	uint8_t userSerialized[Data::User::RequiredBufferSize()] = { 0 };
	...
	if (serializedSize && serializedIndexesSize) {
...
template <Data::CredentialsBits CRED_BIT_MASK> void AccessManager<CRED_BIT_MASK>::LoadUsersFromPersistentStorage()
{
	/* Load all Users from persistent storage */
	uint8_t userIndexesSerialized[UsersIndexes::RequiredBufferSize()] = { 0 };
	uint8_t userData[Data::User::RequiredBufferSize()] = { 0 };
...
template <Data::CredentialsBits CRED_BIT_MASK> void AccessManager<CRED_BIT_MASK>::PrintUser(uint16_t userIndex)
{
	auto *user = &mUsers[userIndex - 1];
