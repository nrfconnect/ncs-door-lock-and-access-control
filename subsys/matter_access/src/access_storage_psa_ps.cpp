...
namespace {
constexpr psa_storage_uid_t kUIDOffset = CONFIG_DOOR_LOCK_MATTER_ACCESS_STORAGE_PROTECTED_STORAGE_UID_OFFSET;

static_assert(static_cast<uint32_t>(kUIDOffset) == 0, "UID offset must fit in the upper 32 bits of the 64-bit UID");
...
psa_storage_uid_t MakeUID(AccessStorage::Type type, uint16_t index, uint16_t subindex)
{
	...
	using CredentialType = CredentialTypeEnum;
	...
	case AccessStorage::Type::User:
		return kUIDOffset + PackIntegers(type, static_cast<UserIndex>(index));
	case AccessStorage::Type::UsersIndexes:
		return kUIDOffset + PackIntegers(type);
	case AccessStorage::Type::Credential:
		return kUIDOffset +
		       PackIntegers(type, static_cast<CredentialType>(index), static_cast<CredentialIndex>(subindex));
	case AccessStorage::Type::CredentialsIndexes:
		return kUIDOffset + PackIntegers(type, static_cast<CredentialType>(index));
	...
	case AccessStorage::Type::YearDaySchedule:
		return kUIDOffset +
		       PackIntegers(type, static_cast<UserIndex>(index), static_cast<ScheduleIndex>(subindex));
	...
	case AccessStorage::Type::HolidaySchedule:
		return kUIDOffset + PackIntegers(type, static_cast<ScheduleIndex>(index));
...
#ifdef CONFIG_DOOR_LOCK_MATTER_ACCESS_PRINT_STORAGE_STATUS
