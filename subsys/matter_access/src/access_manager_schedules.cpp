...
{
	VerifyOrReturnError(userIndex > 0 && userIndex <= CONFIG_DOOR_LOCK_MATTER_ACCESS_MAX_NUM_USERS,
			    DlStatus::kFailure);
	VerifyOrReturnError(weekdayIndex > 0 &&
				    weekdayIndex <=
							  ...
							  uint8_t endMinute)
{
	...
	uint8_t scheduleSerialized[Data::WeekDaySchedule::RequiredBufferSize()] = { 0 };
							  ...
							  EmberAfPluginDoorLockYearDaySchedule &schedule)
{
	VerifyOrReturnError(userIndex > 0 && userIndex <= CONFIG_DOOR_LOCK_MATTER_ACCESS_MAX_NUM_USERS,
			    DlStatus::kFailure);
	VerifyOrReturnError(yearDayIndex > 0 &&
				    yearDayIndex <=
							  ...
							  uint32_t localEndTime)
{
	...
	uint8_t scheduleSerialized[Data::YearDaySchedule::RequiredBufferSize()] = { 0 };
	size_t serializedSize = schedule.Serialize(scheduleSerialized, sizeof(scheduleSerialized));
...
{
	VerifyOrReturnError(holidayIndex > 0 &&
				    holidayIndex <= CONFIG_DOOR_LOCK_MATTER_ACCESS_SCHEDULES_MAX_HOLIDAY_SCHEDULES,
			    DlStatus::kFailure);
							  ...
							  OperatingModeEnum operatingMode)
{
	...
	uint8_t scheduleSerialized[Data::HolidaySchedule::RequiredBufferSize()] = { 0 };
	size_t serializedSize = schedule.Serialize(scheduleSerialized, sizeof(scheduleSerialized));
...
template <Data::CredentialsBits CRED_BIT_MASK> void AccessManager<CRED_BIT_MASK>::LoadSchedulesFromPersistentStorage()
{
	...
	uint8_t scheduleWeekDayData[Data::WeekDaySchedule::RequiredBufferSize()] = { 0 };

	uint8_t scheduleYearDayIndexesSerialized[YearDayScheduleIndexes::ScheduleList::RequiredBufferSize()] = { 0 };
	uint8_t scheduleYearDayData[Data::YearDaySchedule::RequiredBufferSize()] = { 0 };

	for (size_t userIndex = 1; userIndex <= CONFIG_DOOR_LOCK_MATTER_ACCESS_MAX_NUM_USERS; userIndex++) {
		scheduleFound = false;
...
#if CONFIG_DOOR_LOCK_MATTER_ACCESS_DEBUG
					else {
	...
	uint8_t scheduleHolidayData[Data::HolidaySchedule::RequiredBufferSize()] = { 0 };
