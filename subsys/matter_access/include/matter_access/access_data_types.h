...
constexpr size_t kMaxCredentialLength{ CONFIG_DOOR_LOCK_MATTER_ACCESS_MAX_CREDENTIAL_LENGTH };
...
struct Credential {
	 ...
	 * given index is lower than CONFIG_DOOR_LOCK_MATTER_ACCESS_MAX_NUM_CREDENTIALS_PER_TYPE and size of
	 ...
	 */
	CHIP_ERROR FillFromPlugin(const EmberAfPluginDoorLockCredentialInfo &credentialInfo);
...
struct WeekDaySchedule : public Schedule<WeekDaySchedule> {
	 ...
	 */
	CHIP_ERROR FillFromPlugin(const EmberAfPluginDoorLockWeekDaySchedule &plugin);
...
struct YearDaySchedule : public Schedule<YearDaySchedule> {
	 ...
	 * @return CHIP_NO_ERROR if whole struct has been filled properly.
	 */
	CHIP_ERROR FillFromPlugin(const EmberAfPluginDoorLockYearDaySchedule &plugin);
...
struct HolidaySchedule : public Schedule<HolidaySchedule> {
	 ...
	 * @return CHIP_NO_ERROR if whole struct has been filled properly.
	 */
	CHIP_ERROR FillFromPlugin(const EmberAfPluginDoorLockHolidaySchedule &plugin);
...
struct User {
	...
	struct Credentials {
		size_t mSize;
		CredentialStruct mData[CONFIG_DOOR_LOCK_MATTER_ACCESS_MAX_NUM_CREDENTIALS_PER_USER];
	};
	struct Name {
		size_t mSize;
	 ...
	 * @return CHIP_NO_ERROR if whole struct has been filled properly.
	 */
	CHIP_ERROR FillFromPlugin(const EmberAfPluginDoorLockUserInfo &userInfo);
...
constexpr CredentialsBits PIN = ToCredentialBits(CredentialTypeEnum::kPin);
constexpr CredentialsBits RFID = ToCredentialBits(CredentialTypeEnum::kRfid);
constexpr CredentialsBits FINGER = ToCredentialBits(CredentialTypeEnum::kFingerprint);
constexpr CredentialsBits VEIN = ToCredentialBits(CredentialTypeEnum::kFingerVein);
constexpr CredentialsBits FACE = ToCredentialBits(CredentialTypeEnum::kFace);
constexpr CredentialsBits ALIRO_CRED_ISSUER = ToCredentialBits(CredentialTypeEnum::kAliroCredentialIssuerKey);
constexpr CredentialsBits ALIRO_EV_EP = ToCredentialBits(CredentialTypeEnum::kAliroEvictableEndpointKey);
constexpr CredentialsBits ALIRO_NON_EV_EP = ToCredentialBits(CredentialTypeEnum::kAliroNonEvictableEndpointKey);

template <CredentialsBits CRED_BIT_MASK> class Credentials {
public:
	using CredentialOperation = bool (*)(::DoorLock::MatterAccess::Data::Credential &, uint8_t credentialIndex);

	using CredentialArray = std::array<Credential, CONFIG_DOOR_LOCK_MATTER_ACCESS_MAX_NUM_CREDENTIALS_PER_TYPE>;

	CredentialArray &GetCredentialsTypes(CredentialTypeEnum type, bool &success)
	{
		static CredentialArray sDummyObj{};
		if (IsCredentialTypeSupported(type)) {
			success = true;
			return mCredentials[sCredentialIndexArray[chip::to_underlying(type)]];
		}
		success = false;
		return sDummyObj;
	}
	...
	{
		VerifyOrReturnError(IN_RANGE(credentialIndex, 1,
...
private:
	using CredentialIndexArray = std::array<uint8_t, chip::to_underlying(CredentialTypeEnum::kUnknownEnumValue)>;

	static constexpr CredentialIndexArray GetCredentialIndexArray()
	{
		CredentialIndexArray array{};
		uint8_t bits{ 0 };

		for (uint8_t i = 0; i < array.size(); ++i) {
			if (CRED_BIT_MASK & BIT(i)) {
				array[i] = bits++;
			}
		}
	...
	static constexpr bool IsCredentialTypeSupported(CredentialTypeEnum type)
	{
		return (CRED_BIT_MASK & ToCredentialBits(type)) != 0;
	}

	static constexpr uint8_t RequestedNumOfCredTypesSupported() { return POPCOUNT(CRED_BIT_MASK); }

	static_assert(CRED_BIT_MASK <=
			      (PIN | RFID | FINGER | VEIN | FACE | ALIRO_CRED_ISSUER | ALIRO_EV_EP | ALIRO_NON_EV_EP),
	...
	static constexpr CredentialIndexArray sCredentialIndexArray{ GetCredentialIndexArray() };
