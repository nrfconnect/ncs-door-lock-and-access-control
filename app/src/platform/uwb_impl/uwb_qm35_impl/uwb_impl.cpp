						       ...
						       SessionContextHandle sessionContextData)
{
	...
	aliro_uwb_session *sessionCtx =
		aliro_uwb_session_create(mAliroCtx, sessionId, &SessionHandlerCallback, &TransmitBleMessage, this);
	VerifyOrReturnStatus(sessionCtx, ALIRO_NO_MEMORY, LOG_ERR("Failed to create UWB session."));

	AliroError err = ConvertUwbError(aliro_uwb_session_set_ursk(sessionCtx, ursk.data()));
	VerifyOrExit(err == ALIRO_NO_ERROR, LOG_ERR("Failed to set URSK in UWB session: %d", err.ToInt()));

	err = AddSession({ .mUwbSessionContext = sessionCtx, .mSessionContextData = sessionContextData });
...
exit:
	aliro_uwb_session_destroy(sessionCtx);
