...
AliroError UltraWideBandImpl::_ConfigureRangingSession(SessionIdentifier sessionId, const uint8_t *ursk, size_t urskLen,
						       void *sessionUserData)
{
	...
	uwbErr = ALIRO_UWB_ERR_INTERNAL;
	VerifyOrExit(mAliroSessionCtx, LOG_ERR("Failed to create UWB session."));

	uwbErr = aliro_uwb_session_set_ursk(mAliroSessionCtx, ursk);
