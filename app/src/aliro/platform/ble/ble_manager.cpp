...
void BleManager::SecurityChanged([[maybe_unused]] bt_conn *connId, bt_security_t level, enum bt_security_err error)
{
...
AliroError BleManager::Init()
{
	...
	L2capServer::Callbacks l2capCallbacks = {
		.mOnConnected =
			[](bt_conn *conn) {
				AliroStack::Instance().IndicateTransportReady(ConnectionHandle::Ble(conn));
			},
		.mOnDisconnected =
			[](bt_conn *conn) {
				AliroStack::Instance().IndicateTransportLoss(ConnectionHandle::Ble(conn));
			},
		.mOnDataReceived =
			[](bt_conn *conn, uint8_t *data, size_t length) {
				AliroStack::Instance().IndicateDataReceived(ConnectionHandle::Ble(conn),
									    { data, length });
			},
	};
...
AliroError BleManager::StartAdvertising()
{
	MutexGuard lock{ sMutex };
