			     ...
			     CryptoTypes::Signature &signature)
{
	...
	status = psa_sign_message(keyId, PSA_ALG_ECDSA(PSA_ALG_SHA_256), msg, msgLength, signature.data(),
...
AliroError Encrypt(CryptoTypes::KeyId keyId, const uint8_t *plainTxt, size_t plainTxtLength, uint8_t *cipherText)
{
	...
	VerifyOrReturnStatus((cipherText && plainTxt && plainTxtLength == blockSize), ALIRO_INVALID_ARGUMENT);

	size_t outLength{};

	psa_status_t status =
		psa_cipher_encrypt(keyId, algorithm, plainTxt, blockSize, cipherText, blockSize, &outLength);
	VerifyOrReturnStatus(status == PSA_SUCCESS, ALIRO_ERROR_INTERNAL,
