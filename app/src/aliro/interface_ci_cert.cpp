...
AliroError VerifyCertificateSignature(const mbedtls_x509_crt &crt)
{
	...
	return CryptoInstance().VerifySignature(kCredentialIssuerCAPublicKeyId, crt.tbs.p, crt.tbs.len, signatureArray);
}
...
AliroError VerifyCertificate(const mbedtls_x509_crt &crt)
{
	...
	VerifyOrReturnStatus(crt.MBEDTLS_PRIVATE(sig_pk) == MBEDTLS_PK_ECDSA, ALIRO_INVALID_DATA_CONTENT,
			     LOG_ERR("Invalid signature algorithm"));
