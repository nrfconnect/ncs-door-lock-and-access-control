...
int KeyValueStorage::Save(const std::string &keyName, const uint8_t *value, size_t valueLen)
{
	std::string key(kDoorLockBaseKey);
	key += '/';
	key += keyName;
...
int KeyValueStorage::Get(const std::string &keyName, uint8_t *buf, size_t bufLength)
{
