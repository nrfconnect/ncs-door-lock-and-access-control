...
#if CONFIG_NCS_ALIRO_LOG_LEVEL_VALUE > 0

namespace Logging {

/**
 * @brief Writes a formatted log message with a platform log level.
 *
 * @param platformLogLevel The platform log level.
 * @param logFormat The printf-style format string.
 * @param ... Arguments for the format string.
 */
void Log(uint8_t platformLogLevel, const char *logFormat, ...);

/**
 * @brief Logs binary data in hexadecimal format with a platform log level.
 *
 * @param platformLogLevel The platform log level.
 * @param data Pointer to the data buffer.
 * @param size Size of the data buffer.
 * @param str Description string prefixed to the hexdump.
 */
void LogHexdump(uint8_t platformLogLevel, const void *data, size_t size, const char *str);

} // namespace Logging

#endif // CONFIG_NCS_ALIRO_LOG_LEVEL_VALUE > 0
...
/**
 * @brief Queue an opaque stack event for deferred processing.
 *
 * Stack allocates and owns the event object. The application must enqueue the
 * pointer and later pass the same pointer back to `AliroStack::ProcessEvent()`
 * from appropriate execution context (for example: workqueue).
 *
 * @param event Opaque pointer to a stack event object.
 *
 * @return ALIRO_NO_ERROR on success, error code otherwise.
 */
...
/**
 * @brief Lock stack mutex.
 *
 * Mutex storage and backend implementation are application-owned.
 * The stack uses this API to guard access to shared resources.
 */
void Lock();

/**
 * @brief Unlock stack mutex.
 */
void Unlock();
...
/** @brief Opaque handle returned by the OS timer backend. */
using Handle = int;
...
/**
 * @brief Acquire timer slot from application-owned pool.
 *
 * @param callback Callback invoked on timer expiration.
 * @param context User context passed to callback.
 *
 * @return Opaque handle to acquired timer slot, kInvalidHandle on failure.
 */
Handle Acquire(Callback callback, void *context);

/**
 * @brief Release previously acquired timer slot.
 *        If timer is currently running, it is stopped before release.
 *
 * @param handle Timer handle returned by Acquire().
 */
void Release(Handle handle);

/**
 * @brief Start or restart timer with timeout.
 *
 * @param handle Timer handle returned by Acquire().
...
/**
 * @brief Generate signature for a message using Reader private key.
 *
...
/**
 * @brief Encrypt data using Group Resolving Key.
 *
 * @param plainText input raw payload to encrypt.
 * @param plainTextLength input size of the payload.
 * @param cipherText output encrypted payload.
 *
 * @return ALIRO_NO_ERROR on success, error status otherwise.
 */
